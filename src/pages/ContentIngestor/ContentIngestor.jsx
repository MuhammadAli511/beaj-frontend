...
const validateData = async () => {
  ...
  try {
    ...
    const response = await validateSheetData({ sheetId, sheetTitle: tabName, courseId: selectedCourseId })

    if (response.status === 200) {
      addLog("Validation successful!", "success")
      let responseData = response.data.result;
      setValidateResp(responseData);
      ...
      if (responseData.scanResults) {
        if (responseData.scanResults?.length) {
          responseData.scanResults.forEach((item, idx) => {
            addLog(`${item}`, "success")
          })
        }
      }
